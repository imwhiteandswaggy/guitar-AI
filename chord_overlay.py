...
# Colors for each finger (BGR format)
FINGER_COLORS = {
    1: (0, 100, 255),   # Blue (Index)
    2: (0, 255, 0),     # Green (Middle)
    3: (0, 255, 255),   # Yellow (Ring)
    4: (0, 0, 255),     # Red (Pinky)
    0: (128, 128, 128)  # Gray (Thumb)
}

# Standard tuning
NOTES = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B']
OPEN_STRINGS = {1: 4, 2: 11, 3: 7, 4: 2, 5: 9, 6: 4}
...
def apply_perspective_correction(x, y, neck_box, fret_map):
    ...
    if fret_map and len(fret_map) >= 2:
        ...
        if len(sorted_frets) >= 2:
            ...
            tilt_top = (last_fret[1] - first_fret[1]) / neck_width if neck_width > 0 else 0
            tilt_bottom = (last_fret[3] - first_fret[3]) / neck_width if neck_width > 0 else 0
            avg_tilt = (tilt_top + tilt_bottom) / 2
            
            if abs(avg_tilt) > 0.01:
                y_correction = avg_tilt * (x - x1) * 0.3
...
def calculate_position_pixel(string_num, fret_num, neck_box, fret_map, string_positions):
    ...
    if fret_num == 0:
        ...
        if fret_map:
            ...
            if not methods:
                for i, (mapped_fret_num, fret_box) in enumerate(sorted_map):
                    if mapped_fret_num > fret_num and i > 0:
        ...
        if methods:
            ...
            neck_left = neck_box[0]
            neck_width = neck_box[2] - neck_left
            if fret_map:
                sorted_map = sorted(fret_map, key=lambda x: x[0])
                if sorted_map:
                    ...
                    if max_fret > 0:
                        ...
                        fret_x = neck_left + int(neck_width * progress * 0.9)
                    else:
                        fret_x = neck_left + 20
                else:
...
def draw_finger_marker(frame, x, y, finger_num, note_name, is_correct=False, size=22):
    ...
    color = (0, 255, 100) if is_correct else base_color
    ...
    if note_name:
        note_size = cv2.getTextSize(note_name, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)[0]
        ...
        cv2.rectangle(frame,
                     ...
                     (0, 0, 0), -1)
        cv2.putText(frame, note_name, (note_x, note_y),
...
def render_chord_overlay(frame, chord_info, neck_box, fret_map, string_positions,
    ...
    for string_num, fret_num, finger_name in chord_info['fingering']:
